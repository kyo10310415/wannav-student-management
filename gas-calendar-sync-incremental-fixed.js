...
// Notion API設定
const NOTION_TUTOR_API_TOKEN = 'YOUR_NOTION_API_TOKEN'; // Tutor用Notionトークンをここに入力
const NOTION_TUTOR_DB_ID = 'YOUR_TUTOR_DATABASE_ID'; // TutorデータベースIDをここに入力

// 差分更新設定
const INCREMENTAL_DAYS_BACK = 7; // 過去何日分を更新対象にするか（デフォルト: 7日）
const INCREMENTAL_DAYS_FORWARD = 60; // 未来何日分を更新対象にするか（デフォルト: 60日）
...
function getTutorEmailsFromNotion() {
  ...
  // サンプルメールアドレスを表示（最初の5件）
  if (emails.length > 0) {
    Logger.log(`メールアドレスサンプル: ${emails.slice(0, 5).join(', ')}...`);
  }
...
function syncLessonsIncremental() {
  ...
  // 削除されたイベントを検出（更新期間内のみ）
  const rowsToDelete = [];
  existingData.forEach((value, eventId) => {
    const eventDate = new Date(value.data[4]); // レッスン日時
    if (eventDate >= startDate && eventDate <= endDate && !fetchedEventIds.has(eventId)) {
