...
**目的**: スケジュールに参加できない場合に、キャンセルまたはリスケを申請する機能です。
...
5. **月別統計の表示（リーダー以上のみ）**
   ...
   - **NEW**: 各Tutorの出席予定回数・出席率を含む詳細統計
   - 表示内容:
     - 出席予定回数: Google Sheetsから取得したスケジュール情報を集計
     - キャンセル回数: データベースに保存されたキャンセル申請数
     - リスケ回数: データベースに保存されたリスケ申請数
     - 合計回数: キャンセルとリスケの合計
     - **NEW**: 出席率: `(出席予定 - キャンセル - リスケ) / 出席予定 × 100`
   - **NEW**: 色分け表示:
     - 出席率 ≤ 25%: 赤文字（太字）
     - 出席率 ≤ 50%: オレンジ文字（半太字）
     - 出席率 > 50%: 緑文字
     - キャンセル・リスケ・合計回数: 5回以上オレンジ、10回以上赤（従来通り）
   - **NEW**: ソート順: 出席率の昇順（低い順）→ 合計回数の降順（多い順）
   - 前月・来月ボタンで月を切り替え可能
   - クルー権限では表示されない
