...
- ✅ 助っ人依頼作成フォーム
  - レッスン日選択
  - レッスン一覧表示（生徒名検索機能付き）
  - **レッスン時間自動抽出機能（スプレッドシートの「2026/02/08 17:00:00」形式から時間を抽出）**
  - **タイムゾーン修正：JST（日本時間）を正しくUTCに変換してデータベースに保存**
...
タイムゾーン修正を適用するため、Render.com本番環境で以下の手順を実行してください：

1. https://wannav-student-management.onrender.com にアクセス
2. 生徒管理ページまたは予約管理ページへ移動
3. ページを完全にリロード（Ctrl+Shift+R または Cmd+Shift+R）
4. **データ更新ボタン**をクリック（Notionと同期、スプレッドシート同期も実行される）
5. 同期完了まで待つ（3～5分）
6. 助っ人Tutor依頼でレッスン時間が正しく表示されることを確認（例: スプレッドシート「19:00」 → アプリ表示「19時～」）
...
- 日付選択 → レッスン一覧表示 → 生徒名検索
- レッスン選択 → **レッスン時間を自動入力（スプレッドシートから自動抽出）** → 依頼理由・備考・期限入力
...
3. レッスン日を選択
4. 該当する生徒のレッスンを選択
5. **レッスン時間が自動的に入力される（スプレッドシートから抽出、例: 17:00）**
6. 依頼理由・備考・期限を入力
7. 確認画面で内容を確認（レッスン時間は「17時～」形式で表示）
8. 「依頼を確定する」ボタンで送信
