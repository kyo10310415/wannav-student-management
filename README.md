...
- レッスン時間のGoogle Calendar APIからの自動取得（現在は手入力）
- レッスン時間のスプレッドシートからの自動取得（現在は手入力）
...
- 日付選択 → レッスン一覧表示 → 生徒名検索
- レッスン選択 → **レッスン時間入力（必須）** → 依頼理由・備考・期限入力
- 確認画面（レッスン時間も表示） → 依頼確定
...
3. レッスン日を選択
4. 該当する生徒のレッスンを選択
5. **レッスン時間を入力（HH:MM形式、例: 10:00）**
6. 依頼理由・備考・期限を入力
7. 確認画面で内容を確認（レッスン時間も表示される）
8. 「依頼を確定する」ボタンで送信
